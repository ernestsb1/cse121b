...
function getWeather() {
const location = document.getElementById("location").value; // Get the user input from the input field
  
const url = `https://api.openweathermap.org/data/2.5/weather?q=${location}&appid=${apiKey}`; // API endpoint for current weather
      ...
      .then((data) => {
        ...
        fetch(forecastUrl)
          ...
          .then((data) => {
            ...
            forecastData.slice(0, 3).forEach((item) => {
              const forecast = {
